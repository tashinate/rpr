...
-- Drop the old function with max_uses_input parameter to resolve the conflict
DROP FUNCTION IF EXISTS public.generate_license_with_preset(text, text, integer);

-- Verify the correct function exists (this will recreate it if needed)
CREATE OR REPLACE FUNCTION public.generate_license_with_preset(preset text, user_email_input text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  ...
  -- Insert license (removed max_uses column reference)
