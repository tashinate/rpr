-- Fix the authenticate_admin function with proper variable naming
...
DECLARE
  ...
  new_session_expires TIMESTAMP WITH TIME ZONE;
BEGIN
  ...
  -- Generate new session token using gen_random_uuid from extensions
  new_session_token := replace(gen_random_uuid()::text, '-', '');
  new_session_expires := now() + interval '24 hours';
  
  -- Update admin record with new session
  UPDATE public.admin_users
  SET 
    session_token = new_session_token,
    session_expires_at = new_session_expires,
    last_login_at = now(),
    updated_at = now()
